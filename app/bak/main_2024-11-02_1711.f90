...
program main
  ...
  call logger%init( app = 'fortran-electricity_load', file = 'server_db_load.log' )
  ...
  ! Make TEPCO csv
  __EXEC__( 'sqlite3 -header -csv '//trim(sql%database)//&
    ' "SELECT * FROM '//trim(sql%table)//';" > '//trim(wd)//'/'//trim(sql%table)//'.csv' )
